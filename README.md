...
### 6. 🤖 텔레그램 복귀 알림 봇 (Telegram Bot)

부대원들의 복귀 처리와 행정 처리를 텔레그램 채팅방 내에서 명령어 및 자연어로 신속하게 처리할 수 있도록 지원하는 지능형 비서 봇입니다.

*   **대원 인증 및 연동:**
    *   `/auth [실명]`: 텔레그램 계정을 데이터베이스 상의 대원 성명과 매칭하여 인증합니다. (예: `/auth 홍길동`)
    *   `/auth`: 실명을 입력하지 않고 명령어만 실행하면, 현재까지 인증을 완료한 모든 대원 목록을 호출(멘션) 가능한 링크와 함께 출력해 줍니다.
    *   **타인 강제 연동 (답장 기능):** 다른 사용자가 보낸 메시지나 **공유된 연락처(Contact Card)**에 답장(Reply)으로 `/auth [실명]`을 입력하면, 해당 사용자의 텔레그램 ID가 강제로 지정한 실명 대원에게 등록됩니다. (방에 없는 인원을 연락처로 공유하여 대신 등록해 줄 때 매우 유용합니다.)
    *   인증이 완료되면, 봇은 해당 사용자의 텔레그램 ID를 인식하여 본인의 복귀를 바로 보고할 수 있습니다.
*   **복귀 및 복귀 취소 처리:**
    *   `ㅂㄱ` 또는 `ㅂㄱ @[이름]`: 본인 또는 멘션한 대원의 복귀를 즉시 처리합니다.
        *   당일 복귀 예정자 명단(휴가/외박)을 자동으로 대조하여 알맞은 카테고리에 복귀 완료 상태를 기록합니다.
        *   복귀 완료 시마다 현재까지 복귀 완료한 인원을 정렬된 양식("대장님, 금일 외박/휴가 복귀 인원인...")으로 출력합니다.
        *   만약 당일 복귀 예정자가 전원 복귀하면, 설정된 시카(Senior)를 자동으로 호출(@mention)하여 알립니다.
    *   `복귀취소 @[이름]`: 복귀 처리되었던 대원의 복귀 상태를 취소하고 이전 미복귀 상태로 되돌립니다.
*   **복귀 예정 현황 조회:**
    *   `복귀 리스트`: 당일 구글 시트 및 DB의 복귀 예정자 명단을 기반으로 휴가/외박 복귀 대상자 리스트를 포맷하여 출력합니다.
    *   `미복귀`: 당일 복귀 대상자 중 아직 복귀 완료하지 않은 미복귀자 목록을 보여줍니다.
*   **행정 관리 명령어:**
    *   `/senior`: 현재 지정된 시카(Senior)의 이름을 확인하고 텔레그램 연동 여부에 따라 멘션 형태로 보여줍니다.
    *   `/changesenior @[이름]`: 부대의 시카(Senior)를 변경하고 지정합니다.
    *   `/date YYYY-MM-DD` 또는 `/date reset`: (디버그용) 당일 기준 날짜를 임시로 설정하거나 초기화하여 특정 날짜의 복귀 테스트를 수행할 수 있게 합니다.
    *   `/sheet test` 또는 `/sheet prod`: 연동된 스프레드시트 모드를 테스트용 혹은 실제 운영용으로 전환하고, 당일의 복귀 캐시 데이터를 만료시킵니다.
    *   `/sheet`: 현재 연동된 스프레드시트 모드(TEST / PROD) 상태를 조회합니다.
    *   `/reset`: 당일 복귀 보고 데이터를 초기화합니다.
    *   `/help`: 봇에서 지원하는 전체 명령어들의 사용 방법을 도움말 형태로 출력합니다.
    *   `/dev [URL]` 또는 `/dev off`: (개발자용) 운영 서버로 전달되는 텔레그램 메시지를 로컬 터널 URL로 자동 프록시(포워딩) 해줍니다. localtunnel/ngrok 주소만 넣어도 함수 경로를 자동으로 완성해 줍니다. 터널이 닫히면 자동으로 운영 서버 모드로 복귀하므로 별도로 되돌릴 필요가 없습니다.
